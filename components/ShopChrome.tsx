...
export function ShopFooter() {
  return <footer className="bg-[var(--ink)] text-white mt-24"><div className="container-shop py-14 grid gap-12 md:grid-cols-[1.4fr_1fr_1fr_1.3fr]">
    ...
    <div><p className="eyebrow !text-white/45 mb-5">Aide</p><div className="grid gap-3 text-sm text-white/75"><span>Livraison & retours</span><span>Guide des tailles</span><span>Nous contacter</span></div></div>
...
export function ProductCard({ product }: { product: any }) {
  ...
  return <article className="product-card group">
    ...
    {product.stock > 0 && <button onClick={() => addItem(product)} className="mt-4 w-full border-b border-[var(--ink)] pb-2 text-left text-[10px] font-bold uppercase tracking-[.16em] opacity-0 transition-opacity group-hover:opacity-100">Ajouter au panier <ArrowRight size={13} className="inline ml-1"/></button>}
  </article>;
}
