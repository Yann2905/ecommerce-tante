...
import {
  ...
  CheckCircle2, Loader2, MapPin, ChevronRight, Sparkles, Phone
} from 'lucide-react';
          ...
          >
            ...
            <div className="p-10 flex flex-col justify-center bg-white">
              ...
              <motion.h2
                ...
                className="text-4xl font-serif font-black text-[#1A0800] mb-3 leading-tight"
              >
                {product.name}
              </motion.h2>
              <motion.div
              ...
              >
                ...
                {product.discount_price && (
                  <span className="text-xl text-[#B48446]/40 line-through">{product.discount_price} €</span>
                )}
              </motion.div>
              ...
              <motion.button
                whileHover={{ scale: 1.02, backgroundColor: '#8B5E34' }}
                whileTap={{ scale: 0.97 }}
                onClick={handleAdd}
          ...
          >
            ...
            <div className="flex border-b border-[#EAD8C0]">
              {['Panier', 'Livraison', 'Confirmé'].map((s, i) => (
                <div key={i} className={`flex-1 py-2.5 text-center text-[9px] font-black uppercase tracking-widest transition-all ${step === i + 1 ? 'text-[#C9A84C] border-b-2 border-[#C9A84C]' : step > i + 1 ? 'text-[#8B5E34]' : 'text-[#C9A84C]/30'}`}>
                  {s}
                </div>
              ))}
            </div>
              ...
              <AnimatePresence mode="wait">
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1.5">
                            <span className="text-[10px] bg-[#FDF8F2] px-2 py-0.5 rounded-full font-bold text-[#8B5E34]">Qté: {item.quantity}</span>
                          </div>
                        </div>
                        <motion.button
                ...
                {step === 2 && (
                  <motion.div key="delivery" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                    ...
                    {[
                      { placeholder: 'Nom Complet *', field: 'name', type: 'text' },
                      { placeholder: 'WhatsApp *', field: 'phone', type: 'tel' },
                    ].map(({ placeholder, field, type }) => (
                      <motion.input
                        key={field} whileFocus={{ scale: 1.01 }}
                ...
                {step === 3 && (
                  <motion.div key="success" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="h-full flex flex-col items-center justify-center text-center space-y-6 py-12">
                    ...
                    <div>
                      ...
                      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-[#8B5E34] mt-2 text-sm font-medium">
                        Votre commande a été transmise à Emma-Shop.
                      </motion.p>
                    </div>
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="p-4 rounded-2xl border border-[#EAD8C0] w-full" style={{ background: '#FDF8F2' }}>
            ...
            {items.length > 0 && step < 3 && (
              ...
              >
                <div className="flex justify-between items-end mb-5 px-1">
                  ...
                  {/* ✅ Prix en euros */}
                  <motion.span key={totalPrice()} initial={{ scale: 1.2 }} animate={{ scale: 1 }} className="text-3xl font-black text-[#2D1B08]">
                    {totalPrice()} €
                  </motion.span>
                </div>
                <motion.button
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <span>{step === 1 ? 'Suivant' : 'Confirmer la commande'}</span>
                      <ArrowRight size={16} />
                    </>
                  )}
                </motion.button>
    ...
    <div className="min-h-screen text-[#2D1B08]" style={{ background: '#FFFDFB' }}>
      ...
      >
        <div className="max-w-7xl mx-auto h-20 flex items-center gap-4">
          ...
          <div className="flex-1 relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[#C9A84C]/50" size={16} />
            <motion.input
              whileFocus={{ boxShadow: '0 0 0 3px rgba(201,168,76,0.15)' }}
              type="text"
              placeholder="Rechercher une pièce..."
              className="w-full pl-11 pr-4 py-3 rounded-xl border text-sm font-medium outline-none transition-all"
              style={{ background: '#FDF8F2', borderColor: '#F5E6D3' }}
          ...
          >
            ...
            <AnimatePresence>
              {cartCount > 0 && (
                <motion.span
                  key={cartCount} initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}
      ...
      <section ref={heroRef} className="relative min-h-screen flex items-center pt-24 pb-20 px-6 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-24 items-center relative z-10">
          ...
          {/* RIGHT — ✅ Photo de la tante conservée */}
          <motion.div
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide justify-center flex-wrap">
          {categories.map((cat, i) => (
            <motion.button
              ...
              className={`px-7 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all relative overflow-hidden ${activeCategory === cat ? 'text-white shadow-lg' : 'text-[#8B5E34] border border-[#F5E6D3] bg-white hover:border-[#C9A84C]/40'}`}
            >
              {activeCategory === cat && (
                <motion.div
                  layoutId="activePill"
      ...
      {/* ── COLLECTION GRID ── */}
      <section id="collection" className="max-w-7xl mx-auto px-6 pb-20">
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-24 text-center">
            ...
            <p className="text-[#8B5E34] italic">Aucun article trouvé...</p>
          </motion.div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            <AnimatePresence>
              {filtered.map((p, i) => (
                ...
                >
                  <div
                    className="relative aspect-[3/4] rounded-[2.5rem] overflow-hidden border-4 border-white shadow-lg group-hover:-translate-y-4 group-hover:shadow-2xl transition-all duration-500"
                  ...
                  <div className="mt-5 px-2">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      {p.discount_price && (
                        <span className="text-sm text-[#8B5E34]/40 line-through">{p.discount_price} €</span>
                      )}
                    </div>
                  </div>
                </motion.div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-10 relative z-10">
          <div>
            <div className="text-3xl font-serif font-black italic text-[#FFFDFB]">
              Emma-Shop
              <Link href="/login">
                <span className="text-[#C9A84C] cursor-default" title="Accès Admin">.</span>
              </Link>
            </div>
            <p className="text-[#8B5E34] text-xs mt-1">Mode & Élégance Africaine</p>
          </div>
          <motion.div whileHover={{ scale: 1.05 }} className="text-2xl font-black flex items-center gap-4 text-[#FFFDFB]">
