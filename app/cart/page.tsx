...
export default function CartPage() {
  ...
  // InitiateCheckout : une fois à l'arrivée sur le formulaire garni, pas à chaque
  // frappe ni à chaque changement de quantité.
  const checkoutTracked = useRef(false);
  useEffect(() => {
    if (checkoutTracked.current || done || !items.length) return;
    checkoutTracked.current = true;
    trackMarketingEvent('InitiateCheckout', {
      contents: items.map((item) => ({ id: item.productId, quantity: item.quantity, name: item.name, price: item.price })),
      value: totalPrice(),
    });
    sendInternalEvent('checkout_started');
  }, [items, done, totalPrice]);
    ...
    try {
      ...
      const response = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...
          // Autorise (ou non) l'envoi serveur vers Meta/TikTok. Le serveur ne
          // devine pas le consentement : le navigateur seul le connaît.
      ...
      // Purchase : uniquement sur une commande réellement créée (data.created).
      // `created: false` = idempotence, la commande existait déjà → aucun second
      // événement. event_id = order.id, identique à celui envoyé par le serveur,
      // ce qui permet à Meta et TikTok de dédupliquer les deux sources.
      if (data.created && orderId) {
        trackMarketingEvent('Purchase', {
          eventId: orderId,
          contents: items.map((item) => ({ id: item.productId, quantity: item.quantity, name: item.name, price: item.price })),
          value: serverTotal,
        });
        sendInternalEvent('order_created');
      }
