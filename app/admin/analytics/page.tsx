...
export default function AnalyticsPage() {
  ...
  return <AdminChrome eyebrow="Audience & activité" title="Statistiques">
    ...
    <div className="mt-6 flex flex-col gap-3 sm:mt-8 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between"><p className="text-xs leading-5 text-[var(--muted)] sm:text-sm">Données anonymisées · aucun nom ni adresse IP enregistré</p><label className="grid gap-1 sm:contents"><span className="eyebrow sm:hidden">Période analysée</span><select value={days} onChange={(event) => setDays(Number(event.target.value))} aria-label="Période analysée" className="min-h-11 w-full border border-[var(--line)] bg-white px-3 py-2 font-bold sm:min-h-0 sm:w-auto sm:text-xs"><option value={7}>7 derniers jours</option><option value={30}>30 derniers jours</option><option value={90}>90 derniers jours</option></select></label></div>
    {loading ? <div className="grid min-h-48 place-items-center"><Loader2 className="animate-spin text-[var(--olive)]"/></div> : data && <>
      ...
      <div className="mt-5 grid gap-4 sm:mt-6 sm:gap-5 lg:grid-cols-[.9fr_1.1fr]"><section className="min-w-0 bg-white p-4 sm:p-6"><div className="flex items-center gap-3 border-b border-[var(--line)] pb-4"><Globe2 size={18} className="shrink-0 text-[var(--olive)]"/><h2 className="display text-xl sm:text-2xl">Origine des visiteurs</h2></div><div className="mt-3 divide-y divide-[var(--line)]">{data.countries.length ? data.countries.map((country) => <div key={country.country} className="flex items-center justify-between gap-3 py-4 sm:gap-4"><div className="min-w-0"><p className="truncate font-semibold">{countryName(country.country)}</p><p className="mt-1 text-xs text-[var(--muted)]">{country.visitors} visiteur{country.visitors > 1 ? 's' : ''}</p></div><strong className="shrink-0 tabular-nums">{country.visits} vue{country.visits > 1 ? 's' : ''}</strong></div>) : <p className="py-8 text-sm text-[var(--muted)]">Aucune donnée pour cette période.</p>}</div></section><section className="min-w-0 bg-white p-4 sm:p-6"><div className="flex items-center gap-3 border-b border-[var(--line)] pb-4"><Activity size={18} className="shrink-0 text-[var(--olive)]"/><h2 className="display text-xl sm:text-2xl">Activité récente</h2></div><div className="mt-3 divide-y divide-[var(--line)]">{data.events.length ? data.events.map((event, index) => <div key={`${event.created_at}-${index}`} className="flex items-start justify-between gap-3 py-4 sm:gap-4"><div className="min-w-0"><p className="truncate font-semibold">{eventLabels[event.event_type] || event.event_type}</p><p className="mt-1 truncate text-xs text-[var(--muted)]">{event.path} · {event.country_code ? countryName(event.country_code) : 'Pays inconnu'}</p></div><time className="shrink-0 whitespace-nowrap text-[10px] text-[var(--muted)]">{date(event.created_at)}</time></div>) : <p className="py-8 text-sm text-[var(--muted)]">Aucune activité pour cette période.</p>}</div></section></div>
    </>}
  </AdminChrome>;
}
