...
// Le catalogue est rendu par le serveur : il est donc présent dans le HTML dès
// le premier affichage. Cela règle trois choses d'un coup — la position de
// défilement peut être rejouée avant la peinture (plus de saut au retour), le
// LCP n'attend plus un aller-retour réseau, et les produits redeviennent
...
async function loadProducts() {
  try {
    const { data } = await supabaseAdmin
      .from('products')
      .select('id, name, description, price, discount_price, stock, category_id, image_url, gallery, is_active, created_at, categories(name)')
      .eq('is_active', true)
      .order('created_at', { ascending: false });
    return data ?? [];
  } catch {
    // Base indisponible : la page reste servie, catalogue vide.
    return [];
  }
}
